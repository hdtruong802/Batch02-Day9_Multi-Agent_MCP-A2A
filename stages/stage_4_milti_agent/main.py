...
Graph: analyze_law -> route_to_specialists -> parallel [call_tax, call_compliance, call_privacy] -> aggregate -> END
"""
...
def search_privacy_law(query: str) -> str:
    ...
    knowledge = [
        (
            ["gdpr", "europe", "eu", "personal data"],
            "GDPR: fines up to 4% of global annual revenue or EUR 20M. Requires lawful basis "
            "for processing, data subject rights, breach notification within 72 hours.",
        ),
        (
            ["ccpa", "california", "consumer", "privacy"],
            "CCPA/CPRA: fines up to $7,500 per intentional violation. Consumers have right "
            "to know, delete, and opt-out of sale of personal information.",
        ),
        (
            ["breach", "leak", "data", "rò rỉ", "dữ liệu"],
            "Data breach liability: regulatory fines (GDPR/CCPA), class action lawsuits, "
            "contractual indemnification, reputational damage, mandatory breach notification.",
        ),
    ]
    query_lower = query.lower()
    results = []
...
def route_to_specialists(state: LegalState) -> list[Send]:
    """Keyword-based routing: dispatch parallel Send objects to specialist nodes."""
    question_lower = state["question"].lower()
    sends: list[Send] = []

    if any(kw in question_lower for kw in ["tax", "irs", "thuế", "taxes"]):
    ...
    specialist_names = [s.node for s in sends]
    print(f"\n  [Node: route_to_specialists] Dispatching to: {specialist_names}")
    return sends
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        "You are a specialist in GDPR, CCPA, and data protection law with expertise in "
        "data breaches, privacy rights, and cross-border data transfers. "
...
async def main():
    ...
    print("  analyze_law -> [call_tax + call_compliance + call_privacy] -> aggregate -> END")
    print()
